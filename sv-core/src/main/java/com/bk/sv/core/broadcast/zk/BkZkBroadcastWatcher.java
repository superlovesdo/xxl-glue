package com.bk.sv.core.broadcast.zk;
 ...
 */
public abstract class BkZkBroadcastWatcher extends BkZkClient implements Watcher {
    private static final Logger logger = LoggerFactory.getLogger(BkZkBroadcastWatcher.class);

    public BkZkBroadcastWatcher(String zkserver) {
        super(zkserver);
    }
    ...
    protected boolean produce(String path, String data) {
        ...
        logger.info(">>>>>>>>>>> BkZkBroadcastWatcher producer:{}, {}={}", ret, path, data);
        return ret;
    }
    ...
    protected boolean watchTopic(String path) {
        try {
            ...
            boolean ret = stat != null ? true : false;
            logger.info(">>>>>>>>>>> BkZkBroadcastWatcher watchTopic:{}, path:{}", ret, path);
            return ret;
        } catch (KeeperException e) {
            logger.error(e.getMessage(), e);
        } catch (InterruptedException e) {
    ...
    protected void consume(String path, String data) {
        try {
            ...
            logger.info(">>>>>>>>>>> BkZkBroadcastWatcher consume:{}={}", path, data);
        } catch (Exception e) {
            logger.error(e.getMessage(), e);
        }
    }
